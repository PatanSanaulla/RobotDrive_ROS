...
def backtrack(stepObj):
    ...
    pathValues.reverse()
    for index, each in enumerate(pathValues):
        pathValues[index][0] = pathValues[index][0]/100
        pathValues[index][1] = pathValues[index][1]/100

    STEPS = pathValues
    print("length of step_object_list", len(STEP_OBJECT_LIST))
    print("length of the pathvalues", len(STEPS))
    print(STEPS)
...
def getValues():
    global STEPS
    #to return steps to the ros controller
    print("returning values"+str(len(STEPS)))
    return STEPS

try:
    START_POINT = input("Enter the Start Points [x,y,theta] position: ")
    GOAL_POINT = input("Enter the Goal Points [x,y] position: ")
    RPM_LIST = input("Enter the RPM Values [left, right] for the robot: ")
    rpm1 = RPM_LIST[0]
    rpm2 = RPM_LIST[1]
